                                ...
                                ElseIf (pVal.ItemUID = "5") Then
                                    ...
                                    If _retVal = 1 Then
                                        If CType(oForm.Items.Item("12").Specific, SAPbouiCOM.CheckBox).Checked Then
                                            If (oApplication.Utilities.updatePrice(oForm)) Then
                                                oApplication.SBO_Application.MessageBox("Item Price Updated Successfully....", 1, "Ok", "", "")
                                            Else
                                                oApplication.SBO_Application.MessageBox("No Price Updated for Item....", 1, "Ok", "", "")
                                            End If
                                        Else
                                            oApplication.SBO_Application.MessageBox("No Price Updated for Item....", 1, "Ok", "", "")
                                        End If
                                        Dim _blnBarUpdate As Boolean = False
                                        oApplication.Utilities.setData(oForm, CType(oForm.Items.Item("19").Specific, SAPbouiCOM.EditText).Value, _blnBarUpdate)
                                        If CType(oForm.Items.Item("12").Specific, SAPbouiCOM.CheckBox).Checked Then
                                            If _blnBarUpdate Then
                                                oApplication.SBO_Application.MessageBox("Item Bar Codes Updated Successfully....", 1, "Ok", "", "")
                                            Else
                                                oApplication.SBO_Application.MessageBox("No BarCode Updated for Item....", 1, "Ok", "", "")
                                            End If
                                        Else
                                            oApplication.SBO_Application.MessageBox("No BarCode Updated for Item....", 1, "Ok", "", "")
                                        End If
...
#Region "Function"
    Private Sub Initialize(ByVal oForm As SAPbouiCOM.Form)
        Try
            ...
            oDt_Import.ExecuteQuery("Select PCode,[Desc],Color,Size,(Convert(VarChar,PCode) + '-' + Convert(VarChar,Color) + '-' + Convert(Varchar,Size)) As Item ,Barcode1,BarCode2,Quantity,PO_Currency,PO_Price,SP_Currency,SP_Price,WareHouse,PriceList,Brand,CountryOfOrigin From Z_POIM Where 1 = 2 ")
            oGrid = oForm.Items.Item("8").Specific
            oGrid.DataTable = oDt_Import

            oDt_Duplicate = oForm.DataSources.DataTables.Item("Dt_Price")
            oDt_Duplicate.ExecuteQuery("Select (Convert(VarChar,PCode) + '-' + Convert(VarChar,Color) + '-' + Convert(Varchar,Size)) As Item ,PO_Price As 'Old Price',SP_Price As 'New Price',CONVERT(VarChar(1),'N') As 'Check',SP_Currency,PriceList From Z_POIM Where 1 = 2 ")
            ...
            oDt_Duplicate.ExecuteQuery("Select PCode,[Desc],Color,Size,(Convert(VarChar,PCode) + '-' + Convert(VarChar,Color) + '-' + Convert(Varchar,Size)) As Item ,Barcode1,BarCode2,Quantity,PO_Currency,PO_Price,SP_Currency,SP_Price,WareHouse,PriceList,Brand,CountryOfOrigin From Z_POIM Where 1 = 2 ")
